...
const VideoRoom: React.FC<VideoRoomProps> = ({ roomId }) => {
  ...
  useEffect(() => {
    const initializeRoom = async () => {
      try {
        ...
        console.error('Error initializing room:', error.message || error)
    ...
    initializeRoom()
  }, [roomId, router, cameraFacing])

  const updateParticipants = useCallback((presenceState: any) => {
    setParticipants(prev => {
      const next = new Map(prev)
      
      Object.entries(presenceState).forEach(([key, value]: [string, any]) => {
        const presenceData = Array.isArray(value) ? value[0] : value
        if (!next.has(presenceData.user_id)) {
          next.set(presenceData.user_id, {
            id: presenceData.user_id,
            username: presenceData.username,
            displayName: presenceData.display_name,
            audioEnabled: true,
            videoEnabled: true,
            isSpeaking: false,
            joinedAt: presenceData.online_at
          })
        }
      })
  ...
  const handlePresenceJoin = useCallback((payload: { presence: { username: string } }) => {
    const presence = payload.presence || {}
    toast.success(`${presence.username} joined the room`)
    updateParticipants()
  }, [updateParticipants])
